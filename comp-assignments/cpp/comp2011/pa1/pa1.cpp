...
int swapAndRecord(int arr[], int arrLen, int indexA, int indexB, int rotations[], int &rotationLen)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    rotate(arr, arrLen, k[i]);
    rotations[rotationLen++] = k[i];
  }
...
{
  // Task 3 TODO
  int rotations_append[MAX_ROTATIONS];
  int final_rotationLen = 0;
  ...
  for (int i = 0; i < arrLen - 1; i++)
  {
    for (int j = 0; j < arrLen - i - 1; j++)
    {
      if (arr[j] > arr[j + 1])
      {
        ...
        {
          rotations_append[final_rotationLen] = rotations[k];
          final_rotationLen++;
        }
      }
    }
  ...
  for (int i = 0; i < final_rotationLen; i++)
  {
    rotations[i] = rotations_append[i];
  }
  rotationLen = final_rotationLen;
  // End of Task 3 TODO
}
